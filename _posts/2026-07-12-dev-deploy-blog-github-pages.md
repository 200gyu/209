...
> 🧪 **대화형 개발 히스토리 (Vibe R&D 일지)**
> 이 블로그는 **AI 에이전트와의 대화**만으로 배포했습니다. 코드를 몰라도, 문제가 터질 때마다 이렇게 조율했죠:
> - 🗣️ *"배포는 초록불인데 사이트가 안 열려."* → 🤖 *"Pages 소스가 브랜치로 돼 있어요. `GitHub Actions`로 바꾸세요."* → ✅ 해결
> - 🗣️ *"내부 링크가 자꾸 깨져."* → 🤖 *"경로를 하드코딩하지 말고 `post_url`로 거세요. 도메인·경로가 바뀌어도 안전합니다."* → ✅ 해결
>
> 아래는 그렇게 **에러를 하나씩 대화로 넘으며** 완성한 실제 배포 기록입니다. 핵심 함정은 뒤 "실무에서 밟은 함정"에 그대로 정리했습니다.
{: .prompt-tip }
