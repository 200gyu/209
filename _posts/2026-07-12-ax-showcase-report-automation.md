...
> 🧪 **대화형 개발 히스토리 (Vibe R&D 일지)** — 이 레퍼런스 빌드도 AI 에이전트와 **대화하며** 완성했습니다. 예: 🗣️ *"요약이 매번 다르게 나와서 못 믿겠어."* → 🤖 *"출력 형식을 고정하고 예시를 넣어 프롬프트를 v3로 올리죠."* → ✅ 재현성 확보. 그 실험의 전 과정(v1→v3)은 [#3 프롬프트 실험실]({% post_url 2026-07-11-agent-build-03-prompt-lab %})에 낱낱이 공개돼 있습니다.
{: .prompt-tip }
